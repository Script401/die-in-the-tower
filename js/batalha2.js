...
function turnoJogador() {
    ...
    if (iniTurnoAc[0] == 0) {
        ...
        iniTurnoAc[1] = ['buff', Math.floor(Math.random()*4+5)]
        ...
        iniTurnoAc[1] = ['debuff 1', Math.floor(Math.random()*4+4)]
        document.getElementById('ini-acao').style.cssText = 'background-color: green;'
        document.getElementById('ini-acao').innerHTML = iniTurnoAc[1][0]
    }
    addC = 5
...
var load = setInterval(() => {
    ...
    vidIniV = Math.floor(vidIniOrg/120*500)
    ...
    vidIniBol.innerHTML = Math.floor(vidIni/500*120)
