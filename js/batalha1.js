...
function turnoJogador() {
    iniTurnoAc[0] > 4 && iniTurnoAc[0] < 13 ? aprIni - enfIni < -10 ? iniTurnoAc[0] = 15 : iniTurnoAc[0] = 0 : aprIni - enfIni < -10 ? iniTurnoAc[0] = 15 : iniTurnoAc[0] = Math.floor(Math.random()*20)
    escJogOrg = 0
    aprJog - 1 < 0 ? 0 : aprJog--
    enfJog - 1 < 0 ? 0 : enfJog--
    aprIni - 1 < 0 ? 0 : aprIni--
    ...
    if (iniTurnoAc[0] < 5) {
        ...
        iniTurnoAc[1] = ['buff', Math.floor(Math.random()*4+6)]
        ...
        iniTurnoAc[1] = ['debuff 1', Math.floor(Math.random()*4+8)]
...
var load = setInterval(() => {
    ...
    vidIniV = Math.floor(vidIniOrg/80*500)
    ...
    vidIniBol.innerHTML = Math.floor(vidIni/500*80)
